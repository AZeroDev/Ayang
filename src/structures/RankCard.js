...
class RankCard {
    ...
    registerFonts(fontArray) {
        if (fontArray && Array.isArray(fontArray)) {
            fontArray.forEach(font => {
                Canvas.GlobalFonts.registerFromPath(font.path, font.name || font.face?.name);
            });
        } else {
            const fontPath = process.cwd()+"/assets/font";
            [{
                path: fontPath+"/TiltNeon-Regular.ttf",
                name: "TiltNeon",
            }, {
                path: fontPath+"/TiltWarp-Regular.ttf",
                name: "TiltWarp",
            }, {
                path: fontPath+"/NotoSans-Regular.ttf",
                name: "NotoSans",
            }, {
                path: fontPath+"/NotoColorEmoji-Regular.ttf",
                name: "NotoColorEmoji",
            }, {
                path: fontPath+"/Roboto-Regular.ttf",
                name: "Roboto",
            }].forEach(font => {
                Canvas.GlobalFonts.registerFromPath(font.path, font.name || font.face?.name);
            })
     ...
     */
    async build(ops = { fontX: "TiltWarp,NotoColorEmoji", fontY: "NotoSans,NotoColorEmoji" }) {
        ...
        ctx.textAlign = "start";
        const name = shorten(this.data.username.name);
        ...
        if (discrim) {
            ...
            ctx.fillText(`#${discrim.substr(0, 4)}`, ctx.measureText(name).width + 325, 164);
        }
        ...
        ctx.textAlign = "start";
        ctx.fillText("/ " + toAbbrev(this.data.requiredXP.data), 670 + ctx.measureText(toAbbrev(this.data.currentXP.data)).width + 10, 164);
